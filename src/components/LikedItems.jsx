...
function LikedItems(){
    const {likeditems,removefromfav}=useItems();
    const {isdarkmode,Backgroundcolor}=useItems();

    return(
            <div className={`grid md:grid-cols-3 lg:grid-cols-4 gap-3 p-4 ${isdarkmode&&Backgroundcolor} w-full `} >
                ...
                <div key={item.id} className="hover:scale-103">
                    ...
                    <div className="w-full flex-grow justify-between">
                        ...
                        <button onClick={()=>removefromfav(item)} className="w-full cursor-pointer border-1 rounded-full p-3 mt-1">Remove</button>
                    </div>
                </div>     
                ))}
            </div>
